...
typename EA<problem,solution>::population_type EA<problem,solution>::reproduction_step() {
    //define selecao
    population_type parents = selection(this->_population, n_of_selection_candidates(),selection_strategy::uniform);
...
void EA<problem,solution>::population_update_step(population_type& children) {
    ...
    //define selecao
    next_population_candidates = selection(next_population_candidates, this->_population_size - size_of_elite_set,selection_strategy::tournament);
